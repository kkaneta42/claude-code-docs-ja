...
* **PII / 規制対象データの場所**：個人データまたは規制対象データを保持するバケット、データベース、またはパス。分類器がコンテンツから推測する代わりにこれらの場所を保護します
...
内部パッケージレジストリ、PII / 規制対象データの場所、機密リモートターゲット、および保護された IaC スコープエントリには Claude Code v2.1.195 以降が必要です。以前のバージョンはそれらをプレーンコンテキストとして読み込みますが、それらをターゲットにする組み込みルールはありません。
