...
[プラグイン](/docs/ja/plugins-reference#plugin-agent-frontmatter)に含まれるエージェントは、サブエージェントフィールドのサブセットに対応しています。
