...
*Fable* は、最も難しく、最も長時間実行されるタスクに対して最も能力の高いモデルです。これはデフォルトではないため、`/model fable` で選択してください。サイバーセキュリティとバイオロジーコンテンツは自動的に Opus にフォールバックすることに注意してください。Opus 5 は独自のチェックを実行するため、フラグが付いたサイバーセキュリティコンテンツはモデルを切り替え、フラグが付いたバイオロジーコンテンツは拒否されます。
...
| Opus   | 大規模なリファクタリング、複雑なデバッグ、アーキテクチャの決定、高リスクな変更。Opus 5 では、サイバーセキュリティまたはバイオロジーコンテンツは [自動モデルフォールバックまたは拒否](/docs/ja/model-config#automatic-model-fallback) をトリガーします      |
