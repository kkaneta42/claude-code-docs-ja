...
より小さな会話ではなく、より大きなウィンドウが必要な場合、Fable 5、Opus 4.6 以降、および Sonnet 4.6 は 100 万トークンのコンテキストウィンドウをサポートしています。プランごとの利用可能性と `[1m]` モデルバリアントの選択方法については、[拡張コンテキスト](/ja/model-config#extended-context)を参照してください。コンパクションはより大きな制限でも同じ方法で機能します。
