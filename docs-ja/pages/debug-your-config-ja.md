...
`/doctor` を実行して設定とインストールを確認します。無効な設定ファイル、重複したインストール、未使用の拡張機能など、検出した内容をレポートし、確認後にのみ適用される修正を提案します。v2.1.205 より前では、`/doctor` は読み取り専用の診断画面を開き、`f` を押すとレポートが Claude に送信されて修正されました。
