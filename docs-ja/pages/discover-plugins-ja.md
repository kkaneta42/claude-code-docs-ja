...
セッション中にプラグインをインストール、有効化、または無効化すると、一部の変更（新しいコマンドとフックなど）はすぐに有効になります。LSP サーバーの更新を含む他の変更には、リスタートが必要です。

すべての保留中のプラグイン変更をリスタートなしで有効化するには、以下を実行します。

```shell  theme={null}
/reload-plugins
```

Claude Code はすべてのアクティブなプラグインをリロードし、読み込まれたものを報告します。LSP サーバーが追加または更新された場合、それらが有効になるにはリスタートが必要であることが通知されます。
