...
* `/model` の後もエラーが続く場合、1M モデル ID が他の場所に設定されている可能性があります。優先順位順に確認する設定場所については、[選択されたモデルに問題があります](#there%E2%80%99s-an-issue-with-the-selected-model)を参照してください。
...
* v2.1.200 より前に保存されたモデルはこのチェックで修復されません。古い値が戻り続ける場合は、[選択されたモデルに問題があります](#there%E2%80%99s-an-issue-with-the-selected-model) に記載されている場所から削除します
