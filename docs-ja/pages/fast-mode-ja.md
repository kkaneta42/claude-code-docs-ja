...
<Note>
  高速モードは[リサーチプレビュー](#research-preview)段階です。フィーチャー、価格設定、および利用可能性はフィードバックに基づいて変更される可能性があります。
</Note>

高速モードは Claude Opus 4.6 の高速設定で、モデルを 2.5 倍高速化しますが、トークンあたりのコストが高くなります。迅速な反復やライブデバッグなどのインタラクティブな作業で速度が必要な場合は `/fast` でオンに切り替え、コストがレイテンシーより重要な場合はオフに切り替えます。

高速モードは異なるモデルではありません。同じ Opus 4.6 を使用していますが、コスト効率よりも速度を優先する異なる API 設定です。同じ品質と機能が得られ、レスポンスが高速化されるだけです。

知っておくべきこと：

* Claude Code CLI で `/fast` を使用して高速モードをオンに切り替えます。Claude Code VS Code Extension でも `/fast` で利用可能です。
...
* サブスクリプションプラン（Pro/Max/Team/Enterprise）の Claude Code ユーザーの場合、高速モードは追加使用量のみで利用可能で、サブスクリプションレート制限に含まれていません。
...
デフォルトでは、高速モードはセッション全体で保持されます。管理者は高速モードを各セッションでリセットするように設定できます。詳細は[セッションごとのオプトインが必要](#require-per-session-opt-in)を参照してください。
...
| **低い努力レベル** | 思考時間が短い、レスポンスが高速、複雑なタスクでは品質が低い可能性 |
...
<Note>
  高速モード使用量は、プランに残りの使用量がある場合でも、追加使用量に直接請求されます。これは高速モードトークンがプランに含まれる使用量にカウントされず、最初のトークンから高速モード料金で請求されることを意味します。
</Note>
...
高速モードはリサーチプレビューフィーチャーです。これは以下を意味します：

* フィーチャーはフィードバックに基づいて変更される可能性があります
* 利用可能性と価格設定は変更される可能性があります
* 基盤となる API 設定は進化する可能性があります

通常の Anthropic サポートチャネルを通じて問題またはフィードバックを報告してください。

## 関連項目

* [モデル設定](/ja/model-config)：モデルを切り替えて努力レベルを調整する
* [コストを効果的に管理する](/ja/costs)：トークン使用量を追跡してコストを削減する
* [ステータスラインの設定](/ja/statusline)：モデルとコンテキスト情報を表示する
