...
`/goal` は、評価器がフック システムの一部であるため、信頼ダイアログを受け入れたワークスペースでのみ実行されます。[`disableAllHooks`](/ja/hooks#disable-or-remove-hooks) が管理ポリシー設定で設定されている場合、`/goal` は利用できません。どちらの場合も、コマンドは何もしないのではなく、理由を伝えます。
