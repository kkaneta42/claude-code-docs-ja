...
Hook を作成する最速の方法は、Claude Code の `/hooks` インタラクティブメニューを使用することです。このチュートリアルでは、デスクトップ通知 hook を作成するため、Claude がターミナルを監視する代わりに入力を待っているときにアラートを受け取ります。

<Steps>
  <Step title="hooks メニューを開く">
    Claude Code CLI で `/hooks` と入力します。利用可能なすべての hook イベントのリストと、すべての hooks を無効にするオプションが表示されます。各イベントは Claude のライフサイクルのカスタムコードを実行できるポイントに対応しています。`Notification` を選択して、Claude があなたの注意が必要な場合に発火する hook を作成します。
  </Step>

  <Step title="マッチャーを設定する">
    メニューはマッチャーのリストを表示します。マッチャーは hook が発火するタイミングをフィルタリングします。マッチャーを `*` に設定して、すべての通知タイプで発火するようにします。後で `permission_prompt` や `idle_prompt` などの特定の値にマッチャーを変更して、絞り込むことができます。
  </Step>

  <Step title="コマンドを追加する">
    `+ Add new hook…` を選択します。メニューはイベントが発火したときに実行するシェルコマンドを求めます。Hooks は提供するすべてのシェルコマンドを実行するため、プラットフォームの組み込み通知ツールを使用できます。OS のコマンドをコピーします：

    <Tabs>
      <Tab title="macOS">
        AppleScript を通じてネイティブ macOS 通知をトリガーするために [`osascript`](https://ss64.com/mac/osascript.html) を使用します：
      ...
      <Tab title="Linux">
        ほとんどの Linux デスクトップに通知デーモンがプリインストールされている `notify-send` を使用します：
  ...
  <Step title="ストレージの場所を選択する">
    メニューは hook 設定を保存する場所を尋ねます。`User settings` を選択して `~/.claude/settings.json` に保存します。これはすべてのプロジェクトに hook を適用します。現在のプロジェクトにスコープするために `Project settings` を選択することもできます。利用可能なすべてのスコープについては、[Configure hook location](#configure-hook-location) を参照してください。
  </Step>
...
Hooks を使用すると、Claude Code のライフサイクルの重要なポイントでコードを実行できます：編集後にファイルをフォーマットし、実行前にコマンドをブロックし、Claude が入力を必要とするときに通知を送信し、セッション開始時にコンテキストを注入するなど。Hook イベントの完全なリストについては、[Hooks リファレンス](/ja/hooks#hook-lifecycle) を参照してください。
...
Claude が編集するすべてのファイルで [Prettier](https://prettier.io/) を自動的に実行し、手動操作なしでフォーマットを一貫性のあるものにします。
...
<Note>
  このページの Bash の例は JSON 解析に `jq` を使用しています。`brew install jq`（macOS）、`apt-get install jq`（Debian/Ubuntu）でインストールするか、[`jq` ダウンロード](https://jqlang.github.io/jq/download/) を参照してください。
</Note>
...
<Steps>
  ...
  <Step title="hook を登録する">
    `PreToolUse` hook を `.claude/settings.json` に追加して、`Edit` または `Write` ツール呼び出しの前にスクリプトを実行します：
...
Claude のコンテキストウィンドウがいっぱいになると、圧縮は会話を要約してスペースを解放します。これは重要な詳細を失う可能性があります。`SessionStart` hook を `compact` マッチャーで使用して、すべての圧縮後に重要なコンテキストを再注入します。
...
セッション中に設定またはスキルファイルが変更されるときを追跡します。`ConfigChange` イベントは外部プロセスまたはエディタが設定ファイルを変更するときに発火するため、コンプライアンスのために変更をログに記録したり、不正な変更をブロックしたりできます。
...
各 hook には、実行方法を決定する `type` があります。ほとんどの hooks は `"type": "command"` を使用し、シェルコマンドを実行します。他に 3 つのタイプが利用可能です：
...
スクリプトはその JSON を解析し、これらのフィールドのいずれかに基づいて動作できます。`UserPromptSubmit` hooks は `prompt` テキストを取得し、`SessionStart` hooks は `source`（startup、resume、clear、compact）を取得するなど。リファレンスの [共通入力フィールド](/ja/hooks#common-input-fields) で共有フィールドを参照し、各イベントのセクションでイベント固有のスキーマを参照してください。
...
* **終了 2**：アクションがブロックされます。stderr に理由を書き込み、Claude はそれをフィードバックとして受け取り、調整できます。
* **その他の終了コード**：アクションが続行されます。Stderr はログに記録されますが、Claude には表示されません。`Ctrl+O` で詳細モードを切り替えて、トランスクリプトでこれらのメッセージを確認してください。
...
たとえば、`PreToolUse` hook はツール呼び出しを拒否して理由を Claude に伝えるか、ユーザーの承認のためにエスカレートできます：
...
マッチャーがない場合、hook はそのイベントのすべての発生で発火します。マッチャーを使用すると、それを絞り込むことができます。たとえば、すべてのツール呼び出しの後ではなく、ファイル編集の後にのみフォーマッターを実行したい場合は、`PostToolUse` hook にマッチャーを追加します：
...
各イベントタイプは特定のフィールドでマッチします。マッチャーは正確な文字列と正規表現パターンをサポートしています：
...
<Tabs>
  <Tab title="すべての Bash コマンドをログに記録する">
    `Bash` ツール呼び出しのみをマッチさせ、各コマンドをファイルにログに記録します。`PostToolUse` イベントはコマンドが完了した後に発火するため、`tool_input.command` は実行されたものを含みます。Hook は stdin で JSON としてイベントデータを受け取り、`jq -r '.tool_input.command'` はコマンド文字列のみを抽出し、`>>` はログファイルに追加します：
  ...
  <Tab title="セッション終了時にクリーンアップする">
    `SessionEnd` イベントはセッションが終了した理由のマッチャーをサポートしています。この hook は `clear`（`/clear` を実行するとき）でのみ発火し、通常の終了では発火しません：
...
`/hooks` メニューを通じて追加された Hooks は直ちに有効になります。Claude Code が実行中に設定ファイルを直接編集する場合、変更は `/hooks` メニューで確認するか、セッションを再開するまで有効になりません。
...
検証がファイルの検査またはコマンドの実行を必要とする場合は、`type: "agent"` hooks を使用します。プロンプト hooks は単一の LLM 呼び出しを行いますが、エージェント hooks は、条件を返す前にファイルを読み取り、コードを検索し、他のツールを使用できるサブエージェントを生成します。
...
Hook 入力データだけで決定を下すのに十分な場合は、プロンプト hooks を使用します。コードベースの実際の状態に対して何かを検証する必要がある場合は、エージェント hooks を使用します。
...
HTTP hooks は、Web サーバー、クラウド関数、または外部サービスに hook ロジックを処理させたい場合に便利です。たとえば、チーム全体のツール使用イベントをログに記録する共有監査サービスです。
...
ヘッダー値は `$VAR_NAME` または `${VAR_NAME}` 構文を使用した環境変数補間をサポートしています。`allowedEnvVars` 配列にリストされている変数のみが解決されます。他のすべての `$VAR` 参照は空のままです。

<Note>
  HTTP hooks は設定 JSON を直接編集することで設定する必要があります。`/hooks` インタラクティブメニューはコマンド hooks の追加のみをサポートしています。
</Note>
...
* `Stop` hooks はタスク完了時だけでなく、Claude が応答を完了するたびに発火します。ユーザーの割り込みでは発火しません。
...
* マッチャーパターンがツール名と正確にマッチすることを確認します（マッチャーは大文字と小文字を区別します）
...
Claude は無限ループで作業を続けます。

Stop hook スクリプトは、既にトリガーされたかどうかを確認する必要があります。JSON 入力から `stop_hook_active` フィールドを解析し、`true` の場合は早期に終了します：
...
* [セキュリティに関する考慮事項](/ja/hooks#security-considerations)：共有環境または本番環境に hooks をデプロイする前に確認してください
* [Bash コマンドバリデータの例](https://github.com/anthropics/claude-code/blob/main/examples/hooks/bash_command_validator_example.py)：完全なリファレンス実装
