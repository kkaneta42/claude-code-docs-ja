...
Claude Code フックはユーザー定義のシェルコマンドで、Claude Code のライフサイクルの様々なポイントで実行されます。フックは Claude Code の動作に対して決定論的な制御を提供し、LLM に実行を選択させるのではなく、特定のアクションが常に実行されることを保証します。
...
* **自動フォーマット**: すべてのファイル編集後に、.ts ファイルに対して `prettier` を実行し、.go ファイルに対して `gofmt` を実行するなど。
...
<Warning>
  フックを追加する際には、フックのセキュリティ上の影響を考慮する必要があります。フックはエージェントループ中に現在の環境の認証情報を使用して自動的に実行されるためです。
...
Claude Code はワークフロー内の異なるポイントで実行される複数のフックイベントを提供します：

* **PreToolUse**: ツール呼び出しの前に実行されます（ブロック可能）
* **PermissionRequest**: 権限ダイアログが表示されたときに実行されます（許可または拒否可能）
* **PostToolUse**: ツール呼び出しの完了後に実行されます
...
`PreToolUse` フックはツール呼び出しの前に実行され、Claude にフィードバックを提供しながらそれらをブロックできます。

### ステップ 2: マッチャーを追加する

`+ Add new matcher…` を選択して、Bash ツール呼び出しでのみフックを実行します。

マッチャーに `Bash` と入力します。

<Note>`*` を使用してすべてのツールにマッチさせることができます。</Note>
...
その後、REPL に戻るまで `Esc` を押します。フックが登録されました。
...
<Note>
  完全な実装例については、公開コードベースの [bash コマンドバリデータ例](https://github.com/anthropics/claude-code/blob/main/examples/hooks/bash_command_validator_example.py)を参照してください。
</Note>
