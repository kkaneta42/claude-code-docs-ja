...
フックは、Claude Code のライフサイクルの特定のポイントで自動的に実行されるユーザー定義のシェル コマンド、HTTP エンドポイント、または LLM プロンプトです。このリファレンスを使用して、イベント スキーマ、設定オプション、JSON 入出力形式、および非同期フック、HTTP フック、MCP ツール フックなどの高度な機能を検索してください。初めてフックを設定する場合は、代わりに[ガイド](/ja/hooks-guide)から始めてください。

## フック ライフサイクル

フックは Claude Code セッション中の特定のポイントで発火します。イベントが発火し、マッチャーがマッチすると、Claude Code はイベントに関する JSON コンテキストをフック ハンドラーに渡します。コマンド フックの場合、入力は stdin に到着します。HTTP フックの場合、POST リクエスト本体として到着します。ハンドラーは入力を検査し、アクションを実行し、オプションで決定を返すことができます。一部のイベントはセッションごとに 1 回発火し、その他は agentic ループ内で繰り返し発火します。
...
スクリプトは stdin から JSON 入力を読み取り、コマンドを抽出し、`rm -rf` が含まれている場合は `"deny"` の `permissionDecision` を返します。
...
[設定](#configuration)セクションでは完全なスキーマについて説明し、各[フック イベント](#hook-events)セクションでは、コマンドが受け取る入力と返すことができる出力について説明しています。
...
設定ファイル解決の詳細については、[設定](/ja/settings)を参照してください。エンタープライズ管理者は `allowManagedHooksOnly` を使用して、ユーザー、プロジェクト、およびプラグイン フックをブロックできます。[フック設定](/ja/settings#hook-configuration)を参照してください。
...
マッチャーは正規表現であるため、`Edit|Write` はいずれかのツールにマッチし、`Notebook.*` は Notebook で始まるツールにマッチします。マッチャーは Claude Code がフックに stdin で送信する[JSON 入力](#hook-input-and-output)のフィールドに対して実行されます。ツール イベントの場合、そのフィールドは `tool_name` です。各[フック イベント](#hook-events)セクションでは、そのイベントのマッチャー値の完全なセットと入力スキーマをリストしています。
...
`UserPromptSubmit`、`Stop`、`TeammateIdle`、`TaskCompleted`、`WorktreeCreate`、および `WorktreeRemove` はマッチャーをサポートせず、すべての出現で常に発火します。これらのイベントに `matcher` フィールドを追加すると、サイレントに無視されます。
...
* **[HTTP フック](#http-hook-fields)**（`type: "http"`）: イベントの JSON 入力を HTTP POST リクエストとして URL に送信します。エンドポイントはコマンド フックと同じ[JSON 出力形式](#json-output)を使用して、レスポンス本体を通じて結果を通信します。
...
* **[エージェント フック](#prompt-and-agent-hook-fields)**（`type: "agent"`）: Read、Grep、Glob などのツールを使用して条件を検証してから決定を返すことができるサブエージェントをスポーンします。[エージェント ベースのフック](#agent-based-hooks)を参照してください。
...
エラー処理はコマンド フックと異なります。2xx 以外のレスポンス、接続失敗、タイムアウトはすべて、実行を続行できるようにするノンブロッキング エラーを生成します。ツール呼び出しをブロックまたはパーミッションを拒否するには、`decision: "block"` または `permissionDecision: "deny"` を含む JSON 本体を持つ 2xx レスポンスを返します。
...
マッチするすべてのフックは並列で実行され、同一のハンドラーは自動的に重複排除されます。コマンド フックはコマンド文字列で重複排除され、HTTP フックは URL で重複排除されます。ハンドラーは現在のディレクトリで Claude Code の環境で実行されます。`$CLAUDE_CODE_REMOTE` 環境変数はリモート Web 環境で `"true"` に設定され、ローカル CLI では設定されません。
...
<Tabs>
  ...
  <Tab title="プラグイン スクリプト">
    ...
    この例はプラグインにバンドルされたフォーマット スクリプトを実行します。
...
Claude Code で `/hooks` と入力して、インタラクティブ フック マネージャーを開きます。ここで、設定ファイルを直接編集することなく、フックを表示、追加、削除できます。ステップバイステップのウォークスルーについては、ガイドの[最初のフックを設定](/ja/hooks-guide#set-up-your-first-hook)を参照してください。

メニュー内の各フックには、そのソースを示すブラケット プレフィックスが付いています。
...
設定ファイルのフックへの直接編集は即座には有効になりません。Claude Code はスタートアップ時にフックのスナップショットをキャプチャし、セッション全体を通じてそれを使用します。これにより、悪意のある、または偶発的なフック変更がセッション中に有効になることなく、レビューを必要とします。フックが外部で変更された場合、Claude Code は警告し、変更が適用される前に `/hooks` メニューでレビューを要求します。

## フック入出力

コマンド フックは stdin を通じて JSON データを受け取り、終了コード、stdout、stderr を通じて結果を通信します。HTTP フックは同じ JSON を POST リクエスト本体として受け取り、HTTP レスポンス本体を通じて結果を通信します。このセクションでは、すべてのイベントに共通するフィールドと動作について説明します。各イベントのセクション（[フック イベント](#hook-events)の下）には、その特定の入力スキーマと決定制御オプションが含まれています。

### 共通入力フィールド

すべてのフック イベントは、各[フック イベント](#hook-events)セクションで説明されているイベント固有のフィールドに加えて、これらのフィールドを JSON として受け取ります。コマンド フックの場合、この JSON は stdin を通じて到着します。HTTP フックの場合、POST リクエスト本体として到着します。
...
| `permission_mode` | 現在の[パーミッション モード](/ja/permissions#permission-modes): `"default"`、`"plan"`、`"acceptEdits"`、`"dontAsk"`、または `"bypassPermissions"` |
...
フック コマンドからの終了コードは、アクションが進行するか、ブロックされるか、無視されるかを Claude Code に伝えます。
...
**終了 2** はブロッキング エラーを意味します。Claude Code は stdout とそれ内の JSON を無視します。代わりに、stderr テキストが Claude にエラー メッセージとしてフィードバックされます。効果はイベントに依存します。`PreToolUse` はツール呼び出しをブロックし、`UserPromptSubmit` はプロンプトを拒否します。完全なリストについては、[終了コード 2 動作](#exit-code-2-behavior-per-event)を参照してください。

**その他の終了コード** はノンブロッキング エラーです。stderr は詳細モード（`Ctrl+O`）で表示され、実行は続行されます。
...
終了コード 2 は、フックが「停止、これをしないでください」と通知する方法です。効果はイベントに依存します。一部のイベントはブロック可能なアクション（まだ発生していないツール呼び出しなど）を表し、その他は既に発生したか防止できないことを表しているためです。
...
| `Stop`               | はい      | Claude が停止することを防止し、会話を続行                 |
| `SubagentStop`       | はい      | サブエージェントが停止することを防止                       |
| `TeammateIdle`       | はい      | チームメイトがアイドル状態になることを防止（チームメイトが作業を続行）      |
| `TaskCompleted`      | はい      | タスクが完了としてマークされることを防止                     |
| `ConfigChange`       | はい      | 設定変更が有効になることをブロック（`policy_settings` を除く） |
| `PostToolUse`        | いいえ     | stderr を Claude に表示（ツールは既に実行）            |
| `PostToolUseFailure` | いいえ     | stderr を Claude に表示（ツールは既に失敗）            |
| `Notification`       | いいえ     | stderr をユーザーのみに表示                        |
| `SubagentStart`      | いいえ     | stderr をユーザーのみに表示                        |
| `SessionStart`       | いいえ     | stderr をユーザーのみに表示                        |
| `SessionEnd`         | いいえ     | stderr をユーザーのみに表示                        |
| `PreCompact`         | いいえ     | stderr をユーザーのみに表示                        |
| `WorktreeCreate`     | はい      | 0 以外の終了コードはワークツリー作成を失敗させる                |
| `WorktreeRemove`     | いいえ     | 失敗はデバッグ モードでのみログされる                      |
...
* **2xx でプレーン テキスト本体**: 成功、テキストはコンテキストとして追加
* **2xx で JSON 本体**: 成功、コマンド フックと同じ[JSON 出力](#json-output)スキーマを使用して解析
* **2xx 以外のステータス**: ノンブロッキング エラー、実行は続行
* **接続失敗またはタイムアウト**: ノンブロッキング エラー、実行は続行

コマンド フックとは異なり、HTTP フックはステータス コードだけではブロッキング エラーを通知できません。ツール呼び出しをブロックまたはパーミッションを拒否するには、適切な決定フィールドを含む JSON 本体を持つ 2xx レスポンスを返します。
...
<Note>
  フックごとに 1 つのアプローチを選択する必要があります。両方ではありません。終了コードだけで通知するか、終了 0 して構造化制御のために JSON を出力するかです。Claude Code は終了 0 でのみ JSON を処理します。終了 2 の場合、JSON は無視されます。
</Note>

フックの stdout は JSON オブジェクトのみを含む必要があります。シェル プロファイルがスタートアップ時にテキストを出力する場合、JSON 解析に干渉する可能性があります。トラブルシューティング ガイドの[JSON 検証に失敗](/ja/hooks-guide#json-validation-failed)を参照してください。
...
| フィールド            | デフォルト   | 説明                                                                |
| :--------------- | :------ | :---------------------------------------------------------------- |
| `continue`       | `true`  | `false` の場合、フックが実行された後、Claude は完全に処理を停止します。イベント固有の決定フィールドより優先されます |
...
すべてのイベントが JSON を通じた動作のブロックまたは制御をサポートしているわけではありません。サポートするイベントは、その決定を表現するために異なるフィールド セットを使用します。フックを書く前に、このテーブルをクイック リファレンスとして使用してください。
...
| TeammateIdle、TaskCompleted                                                     | 終了コードのみ              | 終了コード 2 がアクションをブロック、stderr がフィードバックとしてフィードバック                   |
| PreToolUse                                                                     | `hookSpecificOutput` | `permissionDecision`（allow/deny/ask）、`permissionDecisionReason` |
| PermissionRequest                                                              | `hookSpecificOutput` | `decision.behavior`（allow/deny）                                 |
| WorktreeCreate                                                                 | stdout パス            | フックは作成されたワークツリーへの絶対パスを出力。0 以外の終了は作成を失敗させる                       |
| WorktreeRemove、Notification、SessionEnd、PreCompact                              | なし                   | 決定制御なし。ログやクリーンアップなどの副作用に使用                                      |

各パターンの実行例を以下に示します。

<Tabs>
  <Tab title="トップレベル決定">
    `UserPromptSubmit`、`PostToolUse`、`PostToolUseFailure`、`Stop`、`SubagentStop`、および `ConfigChange` で使用されます。唯一の値は `"block"` です。アクションが進行することを許可するには、JSON から `decision` を省略するか、JSON なしで終了 0 で終了します。
  ...
  <Tab title="PreToolUse">
    より豊かな制御のために `hookSpecificOutput` を使用します。許可、拒否、またはユーザーへのエスカレーション。ツール入力を実行前に変更したり、Claude 用に追加のコンテキストを注入したりすることもできます。オプションの完全なセットについては、[PreToolUse 決定制御](#pretooluse-decision-control)を参照してください。
  ...
  <Tab title="PermissionRequest">
    `hookSpecificOutput` を使用して、ユーザーに代わってパーミッション リクエストを許可または拒否します。許可する場合、ツールの入力を変更したり、パーミッション ルールを適用して、ユーザーが再度プロンプトされないようにすることもできます。オプションの完全なセットについては、[PermissionRequest 決定制御](#permissionrequest-decision-control)を参照してください。
...
各イベントは Claude Code のライフサイクルの特定のポイントに対応し、フックが実行できます。以下のセクションはライフサイクルに一致する順序で並べられています。セッション セットアップから agentic ループを経由してセッション終了まで。各セクションでは、イベントがいつ発火するか、サポートするマッチャー、受け取る JSON 入力、および出力を通じた動作の制御方法について説明しています。
...
SessionStart はすべてのセッションで実行されるため、これらのフックを高速に保ちます。
...
セットアップ コマンドからのすべての環境変更をキャプチャするには、前後のエクスポートされた変数を比較します。
...
ユーザーがプロンプトを送信するときに実行され、Claude がそれを処理する前です。これにより、プロンプト/会話に基づいて追加のコンテキストを追加したり、プロンプトを検証したり、特定の種類のプロンプトをブロックしたりできます。
...
| フィールド               | 説明                                                                |
| :------------------ | :---------------------------------------------------------------- |
| `decision`          | `"block"` はプロンプトが処理されることを防ぎ、コンテキストから消去します。省略して、プロンプトが進行することを許可します |
...
Claude がツール パラメーターを作成した後、ツール呼び出しを処理する前に実行されます。ツール名でマッチします。`Bash`、`Edit`、`Write`、`Read`、`Glob`、`Grep`、`Task`、`WebFetch`、`WebSearch`、および任意の[MCP ツール名](#match-mcp-tools)。

[PreToolUse 決定制御](#pretooluse-decision-control)を使用して、ツールの使用を許可、拒否、または許可を求めます。

#### PreToolUse 入力

[共通入力フィールド](#common-input-fields)に加えて、PreToolUse フックは `tool_name`、`tool_input`、および `tool_use_id` を受け取ります。`tool_input` フィールドはツールに依存します。
...
| `timeout`           | 数値   | `120000`           | オプションのタイムアウト（ミリ秒）      |
...
| `offset`    | 数値  | `10`                  | 読み取りを開始するオプションの行番号 |
| `limit`     | 数値  | `50`                  | 読み取るオプションの行数       |
...
| `path`    | 文字列 | `"/path/to/dir"` | 検索するオプションのディレクトリ。デフォルトは現在の作業ディレクトリ |
...
| `path`        | 文字列  | `"/path/to/dir"` | 検索するオプションのファイルまたはディレクトリ                                                        |
| `glob`        | 文字列  | `"*.ts"`         | ファイルをフィルタリングするオプションのグロブ パターン                                                   |
...
Web コンテンツをフェッチして処理します。

| フィールド    | タイプ | 例                             | 説明                     |
| :------- | :-- | :---------------------------- | :--------------------- |
| `url`    | 文字列 | `"https://example.com/api"`   | コンテンツをフェッチする URL       |
| `prompt` | 文字列 | `"Extract the API endpoints"` | フェッチされたコンテンツで実行するプロンプト |
...
[サブエージェント](/ja/sub-agents)をスポーンします。
...
| `model`         | 文字列 | `"sonnet"`                 | デフォルトをオーバーライドするオプションのモデル エイリアス |
...
| フィールド                      | 説明                                                                                |
| :------------------------- | :-------------------------------------------------------------------------------- |
| `permissionDecision`       | `"allow"` はパーミッション システムをバイパス、`"deny"` はツール呼び出しを防止、`"ask"` はユーザーに確認を促す             |
...
ユーザーにパーミッション ダイアログが表示されるときに実行されます。
...
PermissionRequest フックは PreToolUse フックのような `tool_name` と `tool_input` フィールドを受け取りますが、`tool_use_id` はありません。オプションの `permission_suggestions` 配列には、ユーザーが通常パーミッション ダイアログで見る「常に許可」オプションが含まれています。違いはフックが発火するタイミングです。PermissionRequest フックはパーミッション ダイアログがユーザーに表示されようとしているときに実行され、PreToolUse フックはパーミッション ステータスに関係なくツール実行前に実行されます。
...
`PermissionRequest` フックはパーミッション リクエストを許可または拒否できます。すべてのフックで利用可能な[JSON 出力フィールド](#json-output)に加えて、フック スクリプトはこれらのイベント固有のフィールドを含む `decision` オブジェクトを返すことができます。

| フィールド                | 説明                                                       |
| :------------------- | :------------------------------------------------------- |
| `behavior`           | `"allow"` はパーミッションを付与、`"deny"` はそれを拒否                    |
| `updatedInput`       | `"allow"` のみ: 実行前にツールの入力パラメーターを変更                        |
| `updatedPermissions` | `"allow"` のみ: パーミッション ルール更新を適用。ユーザーが「常に許可」オプションを選択するのと同等 |
| `message`            | `"deny"` のみ: パーミッションが拒否された理由を Claude に伝える                |
...
`PostToolUse` フックはツールが既に正常に実行された後に発火します。入力には、ツールに送信された引数である `tool_input` と、それが返した結果である `tool_response` の両方が含まれます。両方の正確なスキーマはツールに依存します。
...
| フィールド                  | 説明                                                          |
| :--------------------- | :---------------------------------------------------------- |
| `decision`             | `"block"` は Claude に `reason` でプロンプトを表示。省略してアクションが進行することを許可 |
...
ツール実行が失敗するときに実行されます。このイベントはエラーをスローするか失敗結果を返すツール呼び出しに対して発火します。これを使用して失敗をログ、アラートを送信、または Claude に是正フィードバックを提供します。
...
| `is_interrupt` | 失敗がユーザー割り込みによって引き起こされたかどうかを示すオプションのブール値 |
...
別のマッチャーを使用して、通知タイプに応じて異なるハンドラーを実行します。この設定は Claude がパーミッション承認を必要とするときにパーミッション固有のアラート スクリプトをトリガーし、Claude がアイドル状態になったときに異なる通知をトリガーします。
...
[共通入力フィールド](#common-input-fields)に加えて、Notification フックは通知テキストを含む `message`、オプションの `title`、および発火した種類を示す `notification_type` を受け取ります。
...
Task ツール経由で Claude Code サブエージェントがスポーンされるときに実行されます。エージェント タイプ名でフィルタリングするマッチャーをサポート（`Bash`、`Explore`、`Plan` などの組み込みエージェント、または `.claude/agents/` からのカスタム エージェント名）。
...
[共通入力フィールド](#common-input-fields)に加えて、SubagentStop フックは `stop_hook_active`、`agent_id`、`agent_type`、`agent_transcript_path`、および `last_assistant_message` を受け取ります。`agent_type` フィールドはマッチャー フィルタリングに使用される値です。`transcript_path` はメイン セッションのトランスクリプトで、`agent_transcript_path` はサブエージェント自身のトランスクリプトで、ネストされた `subagents/` フォルダに保存されています。`last_assistant_message` フィールドはサブエージェントの最終応答のテキスト コンテンツを含むため、フックはトランスクリプト ファイルを解析することなくアクセスできます。
...
[共通入力フィールド](#common-input-fields)に加えて、Stop フックは `stop_hook_active` と `last_assistant_message` を受け取ります。`stop_hook_active` フィールドは、Claude Code が既に stop フックの結果として続行している場合は `true` です。この値をチェックするか、トランスクリプトを処理して、Claude Code が無限に実行されるのを防ぎます。`last_assistant_message` フィールドは Claude の最終応答のテキスト コンテンツを含むため、フックはトランスクリプト ファイルを解析することなくアクセスできます。
...
| フィールド      | 説明                                                   |
| :--------- | :--------------------------------------------------- |
| `decision` | `"block"` は Claude が停止することを防止。省略して Claude が停止することを許可 |
...
[エージェント チーム](/ja/agent-teams)チームメイトがそのターンを終了した後、アイドル状態になろうとしているときに実行されます。これを使用して、チームメイトが作業を停止する前に品質ゲートを実施します。例えば、リント チェックの合格を要求したり、出力ファイルが存在することを確認したりします。

`TeammateIdle` フックが終了コード 2 で終了すると、チームメイトは stderr メッセージをフィードバックとして受け取り、アイドル状態になる代わりに作業を続行します。TeammateIdle フックはマッチャーをサポートせず、すべての出現で発火します。
...
TeammateIdle フックは終了コードのみを使用し、JSON 決定制御は使用しません。この例は、チームメイトがアイドル状態になることを許可する前に、ビルド アーティファクトが存在することをチェックします。
...
タスクが完了としてマークされようとしているときに実行されます。これは 2 つの状況で発火します。任意のエージェントが TaskUpdate ツール経由でタスクを明示的に完了としてマークするとき、または[エージェント チーム](/ja/agent-teams)チームメイトが進行中のタスクでそのターンを終了するときです。これを使用して、テストの合格やリント チェックなどの完了基準を実施してから、タスクをクローズできます。

`TaskCompleted` フックが終了コード 2 で終了すると、タスクは完了としてマークされず、stderr メッセージはモデルにフィードバックとしてフィードバックされます。TaskCompleted フックはマッチャーをサポートせず、すべての出現で発火します。

#### TaskCompleted 入力

[共通入力フィールド](#common-input-fields)に加えて、TaskCompleted フックは `task_id`、`task_subject`、およびオプションで `task_description`、`teammate_name`、および `team_name` を受け取ります。
...
| `teammate_name`    | タスクを完了するチームメイトの名前。存在しない可能性があります |
| `team_name`        | チームの名前。存在しない可能性があります            |

#### TaskCompleted 決定制御

TaskCompleted フックは終了コードのみを使用し、JSON 決定制御は使用しません。この例はテストを実行し、失敗した場合はタスク完了をブロックします。
...
セッション中に設定ファイルが変更されるときに実行されます。これを使用して、設定変更を監査したり、セキュリティ ポリシーを実施したり、設定ファイルへの不正な変更をブロックしたりします。

ConfigChange フックは設定ファイル、管理ポリシー設定、およびスキル ファイルの変更に対して発火します。入力の `source` フィールドは、どのタイプの設定が変更されたかを示し、オプションの `file_path` フィールドは変更されたファイルへのパスを提供します。
...
この例はセキュリティ監査のためにすべての設定変更をログします。
...
[共通入力フィールド](#common-input-fields)に加えて、ConfigChange フックは `source` およびオプションで `file_path` を受け取ります。`source` フィールドは変更された設定タイプを示し、`file_path` は変更されたファイルへのパスを提供します。
...
ConfigChange フックは設定変更が有効になることをブロックできます。終了コード 2 または JSON `decision` を使用して変更を防ぎます。ブロックされた場合、新しい設定は実行中のセッションに適用されません。

| フィールド      | 説明                                      |
| :--------- | :-------------------------------------- |
| `decision` | `"block"` は設定変更が適用されることを防止。省略して変更を許可    |
...
`claude --worktree` を実行するか、[サブエージェントが `isolation: "worktree"` を使用](/ja/sub-agents#choose-the-subagent-scope)するときに、Claude Code は `git worktree` を使用して分離されたワーキング コピーを作成します。WorktreeCreate フックを設定する場合、デフォルトの git 動作を置き換え、SVN、Perforce、Mercurial などの異なるバージョン管理システムを使用できます。

フックは作成されたワークツリー ディレクトリへの絶対パスを stdout に出力する必要があります。Claude Code はこのパスを分離されたセッションの作業ディレクトリとして使用します。

この例は SVN ワーキング コピーを作成し、Claude Code が使用するパスを出力します。リポジトリ URL を自分のものに置き換えてください。
...
フックは stdin の JSON 入力からワークツリー `name` を読み取り、新しいディレクトリに新しいコピーをチェックアウトし、ディレクトリ パスを出力します。最後の行の `echo` は Claude Code がワークツリー パスとして読み取るものです。他の出力を stderr にリダイレクトして、パスに干渉しないようにします。
...
WorktreeCreate フックは標準の許可/ブロック決定モデルを使用しません。代わりに、フックの成功または失敗が結果を決定します。`type: "command"` フックのみがサポートされています。

### WorktreeRemove

[WorktreeCreate](#worktreecreate) のクリーンアップ対応。このフックはワークツリーが削除されるときに発火します。`--worktree` セッションを終了して削除することを選択するか、`isolation: "worktree"` を持つサブエージェントが完了するときです。git ベースのワークツリーの場合、Claude は `git worktree remove` で自動的にクリーンアップを処理します。非 git バージョン管理システムの WorktreeCreate フックを設定した場合、WorktreeRemove フックとペアにしてクリーンアップを処理します。なければ、ワークツリー ディレクトリはディスク上に残ります。
...
| 理由                            | 説明                       |
| :---------------------------- | :----------------------- |
| `clear`                       | `/clear` コマンドでセッションがクリア  |
| `logout`                      | ユーザーがログアウト               |
| `prompt_input_exit`           | プロンプト入力が表示されている間にユーザーが終了 |
| `bypass_permissions_disabled` | バイパス パーミッション モードが無効化     |
| `other`                       | その他の終了理由                 |
...
Bash コマンド フック（`type: "command"`）に加えて、Claude Code はプロンプト ベースのフック（`type: "prompt"`）をサポートしており、LLM を使用してアクションを許可またはブロックするかどうかを評価し、エージェント フック（`type: "agent"`）はサブエージェントをスポーンして、決定を返す前に Read、Grep、Glob などのツールを使用して条件を検証できます。すべてのイベントがすべてのフック タイプをサポートしているわけではありません。

3 つのフック タイプ（`command`、`prompt`、および `agent`）すべてをサポートするイベント。
...
この `Stop` フックは、Claude が完了する前に、すべてのタスクが完了しているかどうかを評価するよう LLM に求めます。
...
| フィールド    | 説明                                    |
| :------- | :------------------------------------ |
| `ok`     | `true` はアクションを許可、`false` はそれを防止       |
...
エージェント ベースのフック（`type: "agent"`）はプロンプト ベースのフックのようですが、マルチターン ツール アクセス付きです。単一の LLM 呼び出しの代わりに、エージェント フックはフックの JSON 入力を使用してサブエージェントをスポーンします。エージェント フックはプロンプト ベースのフックと同じイベントをサポートします。

### エージェント フックの仕組み

エージェント フックが発火するとき。

1. Claude Code はプロンプトとフックの JSON 入力を使用してサブエージェントをスポーン
...
エージェント フックは、フック入力データだけでなく、実際のファイルやテスト出力を検査する必要がある場合に便利です。
...
レスポンス スキーマはプロンプト フックと同じです。許可する場合は `{ "ok": true }`、ブロックする場合は `{ "ok": false, "reason": "..." }`。

この `Stop` フックは、Claude が完了することを許可する前に、すべてのユニット テストが合格することを検証します。
...
デフォルトでは、フックは完了するまで Claude の実行をブロックします。デプロイメント、テスト スイート、外部 API 呼び出しなどの長時間実行タスクの場合、`"async": true` を設定して、Claude が作業を続行している間、フックをバックグラウンドで実行します。非同期フックはブロックまたは Claude の動作を制御できません。`decision`、`permissionDecision`、`continue` などのレスポンス フィールドは、制御しようとしたアクションが既に完了しているため、効果がありません。
...
`timeout` フィールドはバックグラウンド プロセスの最大時間（秒）を設定します。指定されない場合、非同期フックは同期フックと同じ 10 分のデフォルトを使用します。

### 非同期フックの実行方法

非同期フックが発火すると、Claude Code はフック プロセスを開始し、完了を待たずにすぐに続行します。フックは同期フックと同じ JSON 入力を stdin で受け取ります。
...
このフックは Claude がファイルを書き込むたびにバックグラウンドでテスト スイートを開始し、テストが完了したら結果を Claude に報告します。このスクリプトを `.claude/hooks/run-tests-async.sh` にプロジェクトに保存し、`chmod +x` で実行可能にします。
...
次に、プロジェクト ルートの `.claude/settings.json` にこの設定を追加します。`async: true` フラグにより、Claude はテストが実行されている間、作業を続行できます。
...
* 非同期フックはツール呼び出しをブロックまたは決定を返すことができません。フックが完了するまでに、トリガーするアクションは既に進行しています。
...
コマンド フックはシステム ユーザーの完全なパーミッションで実行されます。

<Warning>
  コマンド フックはシステム ユーザーの完全なパーミッションでシェル コマンドを実行します。ユーザー アカウントがアクセスできるファイルを変更、削除、またはアクセスできます。フック コマンドを設定に追加する前に、すべてのフック コマンドを確認してテストしてください。
</Warning>
...
`claude --debug` を実行して、フック実行の詳細を確認します。マッチしたフック、終了コード、出力を含みます。詳細モードを `Ctrl+O` で切り替えて、トランスクリプトでフック進行を確認します。
