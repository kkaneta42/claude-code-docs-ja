...
| `Ctrl+L`                                         | プロンプト入力をクリアして画面を再描画                   | 入力されたテキストをクリアし、ターミナルの完全な再描画を強制します。会話履歴は保持されます。ディスプレイが破損したり部分的に空白になった場合に、これを使用して復旧します                                                                                                   |
...
セッション要約はすべてのプランとプロバイダーでデフォルトでオンです。`/config` トグルをオーバーライドするには、[`CLAUDE_CODE_ENABLE_AWAY_SUMMARY`](/ja/env-vars) を `0` または `1` に設定します。要約は非インタラクティブモードでは常にスキップされます。
