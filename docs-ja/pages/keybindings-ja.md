...
Claude Code はカスタマイズ可能なキーボードショートカットに対応しています。`/keybindings` を実行して、`~/.claude/keybindings.json` に設定ファイルを作成または開きます。
...
| フィールド      | 説明                                    |
| :--------- | :------------------------------------ |
| `$schema`  | エディタのオートコンプリート用のオプション JSON Schema URL |
| `$docs`    | オプションのドキュメント URL                      |
| `bindings` | コンテキスト別のバインディングブロックの配列                |

この例では、チャットコンテキストで `Ctrl+E` を外部エディタを開くにバインドし、`Ctrl+U` をアンバインドしています。
...
| `chat:undo`           | Ctrl+\_                  | 最後のアクションを取り消し |
...
| `messageSelector:top`    | Ctrl+Up、Shift+Up、Meta+Up、Shift+K       | トップにジャンプ |
| `messageSelector:bottom` | Ctrl+Down、Shift+Down、Meta+Down、Shift+J | ボトムにジャンプ |
