...
v2.1.144 以降では、`/model` は現在のセッションにのみ適用され、設定に書き込まれません。新しいセッションのデフォルトとして選択を保存するには、ピッカーでハイライトされた行で `d` を押します。これにより、ユーザー設定の `model` フィールドが書き込まれます。管理設定が優先され、次の起動時に再度適用されます。

`--model` フラグと `ANTHROPIC_MODEL` 環境変数も、それらで起動したセッションにのみ適用されます。異なるターミナルで異なるモデルを同時に実行するには、`/model` で切り替えるのではなく、各ターミナルを独自の `--model` フラグで起動します。
