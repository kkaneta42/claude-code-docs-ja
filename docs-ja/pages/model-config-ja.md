...
`/model <name>` を直接入力すると、`Enter` のように動作します。プロジェクトおよび管理設定は引き続き優先され、次の起動時に再度適用されます。{/* min-version: 2.1.196 */}管理者が設定した[組織デフォルトモデル](#organization-default-model)も次の起動時に再度適用されます。
