...
| `OTEL_LOG_TOOL_DETAILS`                             | ツールイベントで MCP サーバー/ツール名とスキル名のログを有効にする (デフォルト: 無効)                   | `1` で有効化                             |
...
* MCP サーバー/ツール名とスキル名はデフォルトではログされません。これはユーザー固有の設定を明かす可能性があるためです。これらを含めるには、`OTEL_LOG_TOOL_DETAILS=1` を設定します
