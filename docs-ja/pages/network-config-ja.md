...
| `github.com`                         | GitHub ホスト型[プラグインマーケットプレイス](/docs/ja/plugin-marketplaces)とプラグインのクローン（公式 Anthropic マーケットプレイスを含む）。HTTPS または SSH 経由。GitHub `owner/repo` ソースを HTTPS のみでクローンするには、[`CLAUDE_CODE_PLUGIN_PREFER_HTTPS=1`](/docs/ja/env-vars) を設定してください                                                                                                                                                                                                                                                                     |
