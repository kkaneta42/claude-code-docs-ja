...
* **Proactive**: Claude は即座に実行し、日常的な決定で一時停止する代わりに合理的な仮定を立て、計画よりもアクションを優先します。これは [オートモード](/ja/permission-modes#eliminate-prompts-with-auto-mode) が適用するより強力な自律実行ガイダンスであり、権限モードを変更せずに機能するため、ツールが実行される前に権限プロンプトが表示されます。
...
* [権限モード](/ja/permission-modes): Proactive スタイルがオートモードとどのように比較されるか
