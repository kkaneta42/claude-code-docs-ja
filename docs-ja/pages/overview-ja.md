...
<AccordionGroup>
  ...
  <Accordion title="指示、スキル、フックでカスタマイズする" icon="sliders">
    [`CLAUDE.md`](/ja/memory)は、プロジェクトルートに追加するマークダウンファイルで、Claude Code はすべてのセッションの開始時に読み取ります。コーディング標準、アーキテクチャの決定、推奨ライブラリ、レビューチェックリストを設定するために使用します。Claude は、作業中に [自動メモリ](/ja/memory#auto-memory)も構築し、ビルドコマンドやデバッグの洞察などの学習内容を保存し、何も書かずにセッション全体で共有します。
  ...
  <Accordion title="どこからでも作業する" icon="globe">
    ...
    * ターミナルセッションを [デスクトップアプリ](/ja/desktop)に渡して、`/desktop` で視覚的な差分確認を行います
...
| Slack からプルリクエストへのバグレポートをルーティングする | [Slack](/ja/slack)                                                                                                 |
