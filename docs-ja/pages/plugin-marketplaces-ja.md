...
| ソース          | タイプ                         | フィールド                              | 注記                                                                                                                                                                               |
| ------------ | --------------------------- | ---------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 相対パス         | `string`（例：`"./my-plugin"`） | なし                                 | マーケットプレイスリポジトリ内のローカルディレクトリ。`./` で始まる必要があります。ただし、[`metadata.pluginRoot` の下に裸の名前を記述する](#relative-paths)場合は除きます。Claude Code はパスを `.claude-plugin/` ディレクトリではなく、マーケットプレイスルートを基準に解決します |
...
チェックがチェックアウトが最新であることを見つけた場合、Claude Code はそれをそのままにします。チェックが新しいコミットを見つけた場合、またはリモートに到達または認証できないために失敗した場合、Claude Code はマーケットプレイスを再度クローンして新しいクローンと交換します。そのクローンが失敗した場合、既存のチェックアウトはそのままです。再クローンは [大規模なリポジトリでタイムアウト](#git-operations-time-out) する可能性があります。
