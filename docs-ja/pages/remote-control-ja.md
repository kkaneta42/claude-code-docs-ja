...
接続に失敗した場合、インジケーターは赤くなり、`/rc failed` と表示されます。下矢印キーで選択して Enter キーを押すと、失敗の理由と却下オプションが表示されます。または、`/remote-control` を再度実行して再試行します。
