...
* **一部のコマンドはローカルのみです**: ターミナルで対話型ピッカーを開くコマンド（`/mcp`、`/plugin`、`/resume` など）はローカル CLI からのみ機能します。テキスト出力を生成するコマンド（`/compact`、`/clear`、`/context`、`/usage`、`/exit`、`/extra-usage`、`/recap`、`/reload-plugins` を含む）はモバイルと Web から機能します。
