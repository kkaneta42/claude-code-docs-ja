...
<Note>
  Bedrock、Vertex AI、Microsoft Foundry では、プロンプトなしの `/loop` は使用メッセージを出力する代わりにメンテナンスプロンプトを実行します。
</Note>
...
<Note>
  Bedrock、Vertex AI、Microsoft Foundry では、`loop.md` は読み込まれず、プロンプトなしの `/loop` は使用メッセージを出力する代わりにメンテナンスプロンプトを実行します。
</Note>
