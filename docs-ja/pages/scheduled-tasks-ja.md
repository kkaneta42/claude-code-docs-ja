...
<Note>
  組み込みメンテナンスプロンプトはまだすべてのユーザーが利用できるわけではなく、Bedrock、Vertex AI、Microsoft Foundry ではサポートされていません。アクティブでない場合、プロンプトなしの `/loop` は使用メッセージを出力します。
</Note>
...
<Note>
  `loop.md` は[組み込みメンテナンスプロンプト](#run-the-built-in-maintenance-prompt)と同じ可用性に従います。メンテナンスプロンプトがアクティブでない場合、プロンプトなしの `/loop` は使用メッセージを出力し、ファイルは読み込まれません。
</Note>
...
[自分のペースモード](#let-claude-choose-the-interval)では、Claude はタスクが確実に完了したら次のウェイクアップをスケジュールしないことで、ループを自分で終了することもできます。固定間隔のループは、停止するか[7 日が経過](#seven-day-expiry)するまで実行し続けます。
