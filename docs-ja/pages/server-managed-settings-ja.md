...
<Note>
  サーバー管理設定はパブリックベータ版であり、[Claude for Teams](https://claude.com/pricing#team-&-enterprise) および [Claude for Enterprise](https://anthropic.com/contact-sales) カスタマー向けに利用可能です。一般提供前に機能が変更される可能性があります。
</Note>
...
サーバー管理設定と[エンドポイント管理設定](/ja/settings#settings-files)は、Claude Code [設定階層](/ja/settings#settings-precedence)の最上位を占めており、ユーザーまたはプロジェクト設定はこれらをオーバーライドできません。両方が存在する場合、サーバー管理設定が優先され、エンドポイント管理設定は使用されません。
...
| API が利用不可                                | キャッシュされた設定が利用可能な場合は適用され、そうでない場合は次の成功したフェッチまで管理設定は適用されません |
...
ランタイム構成の変更を検出するには、[`ConfigChange` フック](/ja/hooks#configchange)を使用して変更をログに記録するか、有効になる前に不正な変更をブロックします。
