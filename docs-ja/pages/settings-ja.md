...
| `workflowKeywordTriggerEnabled`   | {/* min-version: 2.1.157 */}プロンプト内の単語 `workflow` が[動的ワークフロー](/ja/workflows#ask-for-a-workflow-in-your-prompt)をトリガーするかどうか。単語を入力してトリガーしないようにするには `false` に設定します。Ultracode、`/workflows`、および保存されたワークフローコマンドは影響を受けません。デフォルト：`true`。`/config` に**ワークフローキーワードトリガー**として表示されます                                                                                                                        | `false`                                                                                                                         |
