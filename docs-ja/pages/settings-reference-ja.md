...
* **[`forceLoginGatewayUrl`](#forcelogingatewayurl) と [`forceLoginMethod`](#forceloginmethod) の `"gateway"` 値**: Claude Code はサーバー管理設定からこれらを読み取ることはありません。そのため、そこの値は適用されず、MDM ポリシーまたはマネージド設定ファイルで設定された値も隠しません。マシン上の管理者ソースの中で、ポリシーキーを含む最優先度のソースのみがそれらを提供します。サーバー管理設定も存在するかどうかに関わらず。
