...
スキルの説明がコンテキストに読み込まれるため、Claude は利用可能なものを知っています。多くのスキルがある場合、文字予算を超える可能性があります。予算は動的にコンテキストウィンドウの 2% でスケーリングされ、16,000 文字のフォールバックがあります。`/context` を実行して、除外されたスキルに関する警告を確認します。
