...
Bash でファイルを表示することは、コマンドが `cat`、`head`、`tail`、または `sed -n 'X,Yp'` である場合、パイプ、リダイレクト、またはその他のフラグのない単一ファイルに対して、編集前の読み取り要件を満たします。パイプ出力およびその他の Bash コマンドはカウントされず、Claude はこれらの場合は編集前に Read を使用する必要があります。

これは編集の適格性にのみ影響し、権限には影響しません。[Read および Edit 拒否ルール](/ja/permissions#tool-specific-permission-rules)は、Claude Code が `cat`、`head`、`tail`、`sed` などの Bash で認識するファイルコマンドにも適用されますが、Python または Node スクリプトがファイルを自分で開くなど、ファイルを間接的に読み取るまたは書き込む任意のサブプロセスには適用されません。すべてのプロセスをカバーする OS レベルの強制については、[サンドボックスを有効にする](/ja/sandboxing)を参照してください。
