...
* **より広いカバレッジ**: より大規模なレビュアーエージェントのフリートが並行して変更を探索するため、中程度の労力のローカルレビューでは見落とされる可能性のある問題が浮かび上がります
...
PR モードでは、リモートサンドボックスはローカルの作業ツリーをバンドルするのではなく、ホストから直接プルリクエストをクローンします。PR モードは `github.com` 上のリポジトリおよび Claude Code に接続されている Owner が設定した [GitHub Enterprise Server](/ja/github-enterprise-server) インスタンスで機能します。
...
| 深さ    | effort 引数でスケール | 中程度の `/code-review` エンジン | 独立した検証を備えた複数エージェントフリート        |
| 期間    | 数秒から数分         | 数分                       | 約 5 ～ 10 分                    |
