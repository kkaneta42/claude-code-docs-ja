...
* **`Voice connection failed`**: 接続に失敗したため、記録が文字起こしサービスに到達しませんでした。ネットワークを確認して、もう一度試してください。{/* min-version: 2.1.200 */}オーディオをキャプチャしない記録は、このメッセージの代わりに `No audio detected from microphone` を報告します。v2.1.200 より前は、無音マイクは接続失敗を報告する可能性があり、実際の問題が入力デバイスであるのに、ネットワーク問題を示唆していました。
