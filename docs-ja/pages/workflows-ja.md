...
[サブエージェント](/ja/sub-agents)、[スキル](/ja/skills)、ワークフローはすべてマルチステップタスクを実行できます。違いは、計画を保持する者です。

|                  | サブエージェント            | スキル                 | ワークフロー             |
| :--------------- | :------------------ | :------------------ | :----------------- |
| それは何か            | Claude が生成するワーカー    | Claude が従う指示        | ランタイムが実行するスクリプト    |
| 次に何が実行されるかを決定する者 | Claude、ターンごと        | Claude、プロンプトに従う     | スクリプト              |
| 中間結果が存在する場所      | Claude のコンテキストウィンドウ | Claude のコンテキストウィンドウ | スクリプト変数            |
| 繰り返し可能なもの        | ワーカー定義              | 指示                  | オーケストレーション自体       |
| スケール             | ターンごとに委任されたいくつかのタスク | サブエージェントと同じ         | 実行ごとに数十から数百のエージェント |
| 中断               | ターンを再開始             | ターンを再開始             | 同じセッション内で再開可能      |

ワークフローは計画をコードに移動します。サブエージェントとスキルでは、Claude がオーケストレーター です。ターンごとに次に何を生成するかを決定し、すべての結果は Claude のコンテキストに入ります。ワークフロースクリプトはループ、分岐、中間結果自体を保持するため、Claude のコンテキストは最終的な答えのみを保持します。
...
Claude Code はプロンプトで単語をハイライトし、Claude はターンバイターンで処理する代わりにタスク用のワークフロースクリプトを作成します。

実行が必要なことを実行した場合、その後[コマンドとして保存](#save-the-workflow-for-reuse)できます。

Claude Code が意図しないときに単語をハイライトした場合、`alt+w` を押してこのプロンプトでそれを無視するか、ハイライトされた単語の直後にカーソルがある状態でバックスペースを押します。単語がまったくトリガーされないようにするには、`/config` でワークフローキーワードトリガーをオフにします。

### ultracode で Claude に決定させる

Ultracode は、`xhigh` [推論努力](/ja/model-config#adjust-effort-level)と自動ワークフローオーケストレーションを組み合わせた Claude Code 設定です。オンにすると、Claude は各実質的なタスク用にワークフローを計画し、待機しません。
...
ワークフローは多くのエージェントを生成するため、単一の実行は会話で同じタスクを処理するより意味のあるほど多くのトークンを使用できます。実行は他のセッションと同様にプランの使用量とレート制限にカウントされます。`/workflows` からいつでも実行中のワークフローを停止でき、完了した作業を失うことはありません。
