...
Claude for Enterprise 上の Claude Code の ZDR は、Anthropic の直接プラットフォームにのみ適用されます。Amazon Bedrock、Google Vertex AI、または Microsoft Foundry 上の Claude デプロイメントについては、これらのプラットフォームのデータ保持ポリシーを参照してください。
...
| [Artifacts](/ja/artifacts)                            | Anthropic が運用するインフラストラクチャに公開されたページコンテンツを保存する必要があります。 |
